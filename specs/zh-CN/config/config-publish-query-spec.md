...
服务端同时注册 `/nacos/v1/cs/configs` 和 `/v1/cs/configs`。查询响应使用
`text/plain` 返回原始 content，并在 `Content-MD5` 响应头返回当前内容版本；
配置不存在时返回 HTTP 404。发布和删除接受
`application/x-www-form-urlencoded` 表单，删除不存在的配置按幂等成功处理。
...
一期本地存储固定使用 `${data_dir}/config/configs.db` 的 bbolt 数据库。数据库
无法打开时服务启动失败，不回退到内存存储。
