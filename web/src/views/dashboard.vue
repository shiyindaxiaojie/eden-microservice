...
const datePickerShortcuts = computed(() => [
  {
    text: locale.value === 'zh' ? '最近 30 分钟' : 'Last 30m',
    value: () => {
      const end = new Date()
  ...
  {
    text: locale.value === 'zh' ? '最近 1 小时' : 'Last 1h',
    value: () => {
      const end = new Date()
      const start = new Date()
  ...
  {
    text: locale.value === 'zh' ? '最近 6 小时' : 'Last 6h',
    value: () => {
      const end = new Date()
      const start = new Date()
  ...
  {
    text: locale.value === 'zh' ? '最近 24 小时' : 'Last 24h',
    value: () => {
      const end = new Date()
      const start = new Date()
...
const memoryTrendDeltaText = computed(() => {
  ...
  if (delta == null) {
    return locale.value === 'zh' ? '采集中...' : 'Collecting samples'
  }
...
function getEventTypeMeta(type: string) {
  const metas: Record<string, { zh: string; en: string; tone: string }> = {
    ...
    deregister: { zh: '注销', en: 'Deregister', tone: 'red' },
...
function getEventMessage(event: RegistryEvent) {
  ...
  const exactMessages: Record<string, string> = {
    ...
    'Health checker marked instance offline after missed heartbeats': '心跳丢失，健康检查已将其实例标记为下线',
      ...
      <article class="metric-card is-cyan">
        <div class="metric-card-head">
          <div class="metric-card-title">
            ...
            <h3 class="metric-heading">{{ locale === 'zh' ? '服务' : 'Services' }}</h3>
          </div>
      ...
      <article class="metric-card is-slate">
        <div class="metric-card-head">
          <div class="metric-card-title">
            ...
            <h3 class="metric-heading">{{ locale === 'zh' ? '系统' : 'System' }}</h3>
          </div>
        ...
        <div v-if="activePanel === 'events'" class="panel-actions panel-actions--events">
          ...
          <div class="filter-item">
            ...
            <el-date-picker
              ...
              type="datetimerange"
              :start-placeholder="locale === 'zh' ? '开始' : 'Start'"
              :end-placeholder="locale === 'zh' ? '结束' : 'End'"
              size="default"
