...
import (
	...
	consulhelper "eden-microservice/apps/registry/examples/service-discovery/consul/internal/consulapi"
...
func newAuthCenterHandler(reg *consulhelper.Client, instance *consulhelper.ServiceInstance) http.Handler {
	...
	mux.HandleFunc("/api/auth/permissions/", func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, map[string]interface{}{
			...
			"integration":  "github.com/hashicorp/consul/api",
			"transport":    "consul-http-compat",
			"dependencyOk": true,
		})
	})
	return mux
