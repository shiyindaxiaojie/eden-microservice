...
import (
	...
	pb "eden-microservice/apps/registry/api/proto/registry/v1"
...
// Ensure Client implements Registry.
var _ Registry = (*Client)(nil)

// Client implements the native registry client with connection pooling and failover.
type Client struct {
	addresses  []string
	apiKey     string
	datacenter string
...
// New creates a new native registry client with default settings.
...
// NewWithConfig creates a new native registry client with the given config.
func NewWithConfig(cfg *Config) (*Client, error) {
	if cfg == nil {
		return nil, fmt.Errorf("sdk: config required")
	}
	if len(cfg.Addresses) == 0 {
		return nil, fmt.Errorf("sdk: at least one address required")
	}
	...
	if transport != "http" && transport != "grpc" && transport != "quic" {
		return nil, fmt.Errorf("sdk: unsupported transport %q", cfg.Transport)
	}
	...
	if discovery != "auto" && discovery != "static" {
		return nil, fmt.Errorf("sdk: unsupported discovery mode %q", cfg.DiscoveryMode)
	}
	...
	for _, addr := range cfg.Addresses {
		...
		case "grpc":
			if discovery == "static" {
				if isHTTPAddr {
					return nil, fmt.Errorf("sdk: grpc static mode requires direct gRPC addresses, got %s", addr)
				}
				node.GRPCAddr = addr
			} else if isHTTPAddr {
				node.HTTPAddr = addr
		...
		case "quic":
			if discovery == "static" {
				if isHTTPAddr {
					return nil, fmt.Errorf("sdk: quic static mode requires direct QUIC addresses, got %s", addr)
				}
				node.QUICAddr = addr
			} else if isHTTPAddr {
				node.HTTPAddr = addr
	...
	if len(initialNodes) == 0 {
		return nil, fmt.Errorf("sdk: at least one non-empty address required")
	}
...
func (c *Client) Deregister(instance *ServiceInstance) error {
...
func (c *Client) DiscoveryGroup(serviceName, group string) ([]*ServiceInstance, error) {
	...
	err := c.executeWithFailover("Discovery", func(node *registryNode) error {
		...
		if err := json.NewDecoder(resp.Body).Decode(&serviceRows); err != nil {
			return err
		}

		result := make([]*ServiceInstance, 0, len(serviceRows))
		for _, ei := range serviceRows {
...
func (c *Client) Heartbeat(instance *ServiceInstance) error {
...
func (c *Client) post(path string, body interface{}) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return fmt.Errorf("registry %s: status %d, body: %s", path, resp.StatusCode, string(respBody))
	}
	return nil
}
...
func fromProtoInstances(pbList []*pb.ServiceInstance) []*ServiceInstance {
	result := make([]*ServiceInstance, 0, len(pbList))
