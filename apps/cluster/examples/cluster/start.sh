...
go build -o registry-server ./apps/server/cmd/server
if [ $? -ne 0 ]; then
...
./registry-server -config apps/cluster/examples/cluster/configs/node1.yaml > /tmp/registry-node1.log 2>&1 &
NODE1_PID=$!
sleep 3

echo "  Node 2 (HTTP: 8501)..."
./registry-server -config apps/cluster/examples/cluster/configs/node2.yaml > /tmp/registry-node2.log 2>&1 &
NODE2_PID=$!
sleep 2

echo "  Node 3 (HTTP: 8502)..."
./registry-server -config apps/cluster/examples/cluster/configs/node3.yaml > /tmp/registry-node3.log 2>&1 &
NODE3_PID=$!
sleep 2
