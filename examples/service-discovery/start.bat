...
echo [1/4] Starting Eden Registry...
start "Eden-Registry" cmd /c "go run ./cmd/server/main.go"
timeout /t 3 /nobreak >nul

echo [2/4] Starting User Center (port 9001)...
start "User-Center" cmd /c "go run ./examples/service-discovery/cmd/user-center/main.go"
timeout /t 2 /nobreak >nul

echo [3/4] Starting Auth Center (port 9002)...
start "Auth-Center" cmd /c "go run ./examples/service-discovery/cmd/auth-center/main.go"
timeout /t 2 /nobreak >nul

echo [4/4] Starting Order Center (port 9003)...
...
taskkill /fi "windowtitle eq Eden-Registry" /im cmd.exe /t /f >nul 2>&1
